---
name: best-practice.create
description: '技術ツールのベストプラクティスをまとめた instructions ファイルを作成'
...
# ベストプラクティス instructions ファイルの作成

指定された技術ツールに関するベストプラクティスをまとめた GitHub Copilot 用 instructions ファイルを作成します。
...
- 指示内容: #file:official.prompt.md を `tool=${input:tool} officialUrl=${input:officialUrl}` で実行し、`docs/best_practice/${input:tool}/official.md` を作成
- ${input:additionalOfficialUrls} が指定されている場合は `additionalUrls=${input:additionalOfficialUrls}` も渡す

### 2. コミュニティのベストプラクティスの収集
- #tool:agent/runSubagent を使用してサブエージェントに委譲
- 指示内容: #file:community.prompt.md を `tool=${input:tool}` で実行し、`docs/best_practice/${input:tool}/community.md` を作成
- ${input:additionalCommunityUrls} が指定されている場合は `additionalUrls=${input:additionalCommunityUrls}` も渡す

### 3. instructions ファイルの作成
- #tool:agent/runSubagent を使用してサブエージェントに委譲
- 指示内容: #file:create-instructions.prompt.md を `tool=${input:tool}` で実行し、`.github/instructions/<kebab-case(${input:tool})>.best-practice.instructions.md` を作成